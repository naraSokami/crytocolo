...
function PlaneThatFlies() {
  ...
  useFrame(() => {
    ...
    ref.current.rotation.y = THREE.MathUtils.lerp(curY, nextY, 0.1);
  });

  const glb = useGLTF("/models/plane.glb", true)
  return <primitive ref={ref} object={glb.scene} dispose={null} scale={.03} rotation={[1.5, Math.PI, 0]} />
}
    ...
    <>
      ...
      <Suspense fallback={<Loader />}>
          ...
          <Block factor={1.5} offset={3}>
            {/* <Content /> */}
            <mesh onClick={() => console.log('Hellooo')} >
              <boxGeometry />
